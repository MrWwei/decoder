...
class PullFramer {
  public:
    ...
    using onGetFrame = std::function<
        void(const FrameData::Ptr&, void* userdata, int instance_id)>;
    ...
    void setOnGetFrame(const onGetFrame& onGetFrame,
                       void*             userdata,
                       int               instance_id);
    PullFramer();
    ~PullFramer();
