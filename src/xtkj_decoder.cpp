...
public:
    LocalVideoReader() : formatContext_(nullptr), codecContext_(nullptr),
    ...
    bool open(const std::string& videoPath) {
        if (avformat_open_input(&formatContext_, videoPath.c_str(), nullptr, nullptr) != 0) {
        ...
        AVRational time_base = formatContext_->streams[videoStreamIndex_]->time_base;
        time_base_ = av_q2d(time_base) * 1000.0;  // Convert to milliseconds
        last_pts_ = 0;
        
        // Get FPS
        AVRational frame_rate = formatContext_->streams[videoStreamIndex_]->avg_frame_rate;
        if (frame_rate.den != 0) {
        ...
        if (duration != AV_NOPTS_VALUE && fps_ > 0) {
            total_frames_ = static_cast<int64_t>(duration * fps_ * time_base_ / 1000.0);
        ...
        log_info(std::string("Local video opened: ") + std::to_string(width_) + "x" + std::to_string(height_));
        opened_ = true;
        return true;
    }
        ...
        while (!stop_.load()) {
            while (av_read_frame(formatContext_, packet_) >= 0) {
                ...
                if (packet_->stream_index == videoStreamIndex_) {
                    ...
                    cv::Mat mat(height_, width_, CV_8UC3, frameRGB_->data[0], frameRGB_->linesize[0]);
                    ...
                    if (frame_->pts != AV_NOPTS_VALUE) {
                        ...
                        // If no PTS available, estimate based on frame count and fps
                        last_pts_ += static_cast<int64_t>(1000.0 / av_q2d(formatContext_->streams[videoStreamIndex_]->r_frame_rate));
                    }
    ...
    int64_t getLastPTS() const { return last_pts_; }
    double getFPS() const { return fps_; }
    int64_t getBitrate() const { return bitrate_; }
    int64_t getTotalFrames() const { return total_frames_; }
...
typedef struct
{
    void* puller;
    void* decoder_ffmpeg;
    void* decoder_instance;  // Pointer to Decoder object for updating fps/bitrate
...
int Decoder::start_pull(string video_path, int is_mpp, int interval)
{
    ...
    if (is_local_file_) {
        ...
        worker_ = std::make_shared<std::thread>(&Decoder::process_video, this, &app_ctx_,
                                                video_path.c_str(), std::ref(pro));
    }
...
int Decoder::process_local_video(const char* path, std::promise<bool>& pro)
{
    ...
    if (!local_video_reader_->open(path)) {
        log_error(std::string("Failed to open local video file: ") + std::string(path));
        decoder_status_.store(DECODER_STATUS_FAILED);
        pro.set_value(false);
        return -1;
    }
    ...
    if (err_code == 0) {
        ...
        for (i = 0; i < track_count; ++i) {
            if (mk_track_is_video(tracks[i])) {
                // log_info("Got video track: " + std::string(mk_track_codec_name(tracks[i])));
                ...
                if (ctx->decoder_instance) {
                    ...
                    if (fps_mk > 0) {
                        ...
                        // log_info("RTSP video FPS: " + std::to_string(fps));
                        // std::cout << "[INFO] RTSP video FPS: " 
                        //           << fps << " fps" << std::endl;
                    }
                    
                    // Get bitrate from track (in bps)
                    int bitrate = mk_track_bit_rate(tracks[i]);
                    // std::cout << "[INFO] RTSP video bitrate: " 
                    //               << static_cast<double>(bitrate*10000000)
                    //               << " Mbpsss" << std::endl;
                    if (bitrate > 0) {
                        decoder_obj->set_bitrate(static_cast<int64_t>(bitrate));
                        // log_info("RTSP video bitrate: " + std::to_string(bitrate / 1000000.0) + " Mbpsss");
                        // std::cout << "[INFO] RTSP video bitrate: " 
        ...
        // log_error("Play failed: code=" + std::to_string(err_code) + 
